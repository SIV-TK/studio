...
export default function PatientPortalPage() {
  ...
  const loadPatientData = async () => {
    ...
    setLoading(true);
    try {
      const data = await pharmacyService.getPatientPortalData(session.uid);
      setPortalData(data);
    } catch (error) {
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">
              My Medication Portal
            </h1>
            <p className="text-xl text-gray-600">
              Track medications, reminders, and health education
            </p>
              ...
              {portalData.educationalContent.map((content) => (
                <Card key={`${content.medicationName}_${content.type}`}>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {content.medicationName} • {content.type.charAt(0).toUpperCase() + content.type.slice(1)} Information
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
